...
--
-- Name: svjur; Type: DATABASE; Schema: -; Owner: vinalia
--

CREATE DATABASE svjur WITH TEMPLATE = template0 ENCODING = 'UTF8';


ALTER DATABASE svjur OWNER TO vinalia;
...
CREATE TABLE wines (
    ...
    attribute_id integer NOT NULL,
    category_id integer NOT NULL,
    "year" integer DEFAULT 2008 NOT NULL,
    note text,
    sort_order integer,
    group_id integer,
    CONSTRAINT wines_year_check CHECK ((("year" > 1900) AND ("year" <= 2100)))
);
...
CREATE VIEW results_per_variety AS
    SELECT wines.wine_id AS id, producers.family_name AS producer_family_name, producers.name AS producer_name, producers.address AS producer_address, varieties.name AS variety, colors.name AS color, attributes.name AS attribute, categories.name AS category, wines."year", wines.note, wines.sort_order AS sort_value, wines_scores.score, CASE WHEN (wines_scores.score >= (90)::double precision) THEN 'ZM'::text WHEN ((wines_scores.score >= (85)::double precision) AND (wines_scores.score < (90)::double precision)) THEN 'SM'::text WHEN ((wines_scores.score >= (75)::double precision) AND (wines_scores.score < (85)::double precision)) THEN 'BM'::text ELSE ''::text END AS medal, varieties.sort_order AS catalog_order FROM (((((((wines LEFT JOIN producers USING (producer_id)) LEFT JOIN varieties USING (variety_id)) LEFT JOIN colors USING (color_id)) LEFT JOIN attributes USING (attribute_id)) LEFT JOIN categories USING (category_id)) LEFT JOIN wines_scores USING (wine_id)) LEFT JOIN wines_statistics USING (wine_id)) ORDER BY varieties.sort_order, wines_scores.score DESC, wines_statistics.mad, wines_statistics.stddev;
...
CREATE VIEW wines_groups AS
    SELECT groups.name AS group_name, wines.wine_id, varieties.name AS wine_name, categories.name AS category_name, wines."year" AS wine_year FROM (((wines LEFT JOIN varieties USING (variety_id)) LEFT JOIN categories USING (category_id)) LEFT JOIN groups USING (group_id)) ORDER BY groups.sort_order, varieties.sort_order, categories.sort_order, wines."year" DESC;
...
--
-- Name: attributes_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY attributes
    ADD CONSTRAINT attributes_sort_order_key UNIQUE (sort_order);
...
--
-- Name: categories_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--
...
--
-- Name: colors_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY colors
    ADD CONSTRAINT colors_sort_order_key UNIQUE (sort_order);
...
--
-- Name: groups_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY groups
    ADD CONSTRAINT groups_sort_order_key UNIQUE (sort_order);
...
--
-- Name: judges_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY judges
    ADD CONSTRAINT judges_sort_order_key UNIQUE (sort_order);
...
--
-- Name: varieties_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY varieties
    ADD CONSTRAINT varieties_sort_order_key UNIQUE (sort_order);
...
--
-- Name: wines_sort_order_key; Type: CONSTRAINT; Schema: public; Owner: vinalia; Tablespace: 
--

ALTER TABLE ONLY wines
    ADD CONSTRAINT wines_sort_order_key UNIQUE (sort_order);
